...
create policy "Users can update their created comments." on comments
  for update using (auth.uid() = author);

create policy "Users can delete their created comments." on comments
  for delete using (auth.uid() = author);

-- Prevent the author field of a created species from being changed
create function public.comments_columns_updateable()
returns trigger as $$
begin
  if new.author <> old.author then
    raise exception 'changing comments author is not allowed';
  end if;
  return new;
end;
$$ language plpgsql security definer;
create trigger columns_updateable
  ...
  for each row execute procedure public.comments_columns_updateable();
